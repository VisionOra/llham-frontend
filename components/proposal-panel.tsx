...
import { Download, FileText, FileCode, FileType, Maximize2, Minimize2, Edit, History, X, Save, Loader2 } from "lucide-react"
...
<html>
  ...
  if (!showProposalPanel || !proposalHtml) {
    return null
  }
              ...
              <>
                {isEditMode ? (
                  <Button
                    ...
                    disabled={isSaving}
                    size="sm"
                    className="bg-green-600 hover:bg-green-700 text-white flex-shrink-0 h-8 sm:h-9 px-2 sm:px-3"
                  >
                    {isSaving ? (
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-4 md:p-6 bg-white">
          ...
          {showEditHistory ? (
            <div className="space-y-4">
              ...
              {loadingEdits ? (
                <div className="text-center py-8 text-gray-500">Loading edits...</div>
              ) : proposalEdits.length === 0 ? (
                <div className="text-center py-8 text-gray-500">No edits yet. Use Edit button to make changes.</div>
              ) : (
                <div className="space-y-4">
                  {proposalEdits.map((edit) => (
                    <div key={edit.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-gray-700">{edit.section_identifier}</span>
            ...
            <>
              {isEditMode && (
                <div className="mb-3 text-sm text-blue-600 bg-blue-50 border border-blue-200 rounded p-2">
